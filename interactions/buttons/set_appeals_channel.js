  ...
  async execute(interaction) {
    try {
      ...
      const channelSelectMenu = new ChannelSelectMenuBuilder()
        .setCustomId('appeals_channel_menu')
        .setPlaceholder('Select a channel for appeals')
        .setChannelTypes(ChannelType.GuildText);

      const actionRow = new ActionRowBuilder().addComponents(channelSelectMenu);

      await interaction.reply({
        content: 'Select the channel where you want appeals to be sent:',
        components: [actionRow],
        ephemeral: true,
      });

    } catch (error) {
      logger.error('Error in set_appeals_channel button:', error);
